...
use crate::{DefaultTextStyle, U8g2TextStyle, FONT_LARGE, LEGACY_FONT_LARGE};
use alloc::{
    rc::Rc,
...
type PlaceholderText = alloc::boxed::Box<Container<Align<TextWidget>>>;
...
impl InputPreview {
    pub fn new() -> Self {
        ...
        let hint_switcher =
            alloc::boxed::Box::new(FadeSwitcher::new(None, 300, PALETTE.background));
    ...
    pub fn update_from_view_state(&mut self, view_state: &ViewState) {
        ...
        self.placeholder_text = hint_text.map(|text| {
            let text_widget = TextWidget::new(
                text,
                DefaultTextStyle::new(FONT_LARGE, PALETTE.surface_variant),
            )
