...
/// The title and address are pre-rendered together in a single framebuffer for fast blitting.
#[derive(frostsnap_macros::Widget)]
pub struct AddressPage {
    #[widget_delegate]
    center: Center<Padding<AddressDisplay>>,
}

impl AddressPage {
    fn new_with_seed(index: usize, address: &bitcoin::Address, rand_seed: u32) -> Self {
        let title = format!("To Address #{}", index + 1);
        ...
        let address_display = AddressDisplay::new_with_seed(&title, address.clone(), mixed_seed);

        let padded = Padding::only(address_display).bottom(40).build();
