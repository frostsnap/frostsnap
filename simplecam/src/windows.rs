...
static INITIALIZED: once_cell::sync::Lazy<Arc<AtomicBool>> =
    once_cell::sync::Lazy::new(|| Arc::new(AtomicBool::new(false)));
static CAMERA_REFCNT: once_cell::sync::Lazy<Arc<AtomicUsize>> =
...
fn get_preferred_format(activate: &IMFActivate) -> Result<(FrameFormat, Resolution)> {
    unsafe {
            ...
            if let Some(ff) = frame_format {
                if let Ok(res_u64) = media_type.GetUINT64(&MF_MT_FRAME_SIZE) {
                    ...
                    if ff == FrameFormat::MJPEG && is_better(&best_mjpeg) {
                        best_mjpeg = Some((ff, resolution));
                    } else if ff != FrameFormat::MJPEG && is_better(&best_other) {
                        best_other = Some((ff, resolution));
                    }
                }
            }
        }
        ...
        best_mjpeg
            .or(best_other)
            .ok_or_else(|| CameraError::UnsupportedFormat("No supported format found".to_string()))
    }
}
