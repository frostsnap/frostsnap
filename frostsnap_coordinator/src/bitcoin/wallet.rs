    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PsbtValidationError::NothingToSign {
                ...
                let input_word = if *total_inputs == 1 {
                    "input"
                } else {
                    "inputs"
                };
                write!(
                    f,
                    "This PSBT has {} {} but this wallet can not sign any of them",
                    total_inputs, input_word
                )?;

                if *foreign_count > 0 && *already_signed_count > 0 {
                    write!(
                        f,
                        " ({} not owned by this wallet, {} already signed)",
                        foreign_count, already_signed_count
                    )?;
                } else if *foreign_count > 0 {
                    write!(f, " ({} not owned by this wallet)", foreign_count)?;
                } else if *already_signed_count > 0 {
                    write!(f, " ({} already signed)", already_signed_count)?;
                }
