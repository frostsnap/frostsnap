        ...
        let connect_fut = async {
            let (is_ssl, socket_addr) = match url.split_once("://") {
                ...
                Some((unknown_scheme, _)) => {
                    return Err(TofuError::Other(anyhow!("unknown url scheme '{unknown_scheme}'")));
                }
            ...
            if is_ssl {
                ...
                let stream = connect_with_tofu(&socket_addr, &host, trusted_certificates, db).await?;
                let (mut rh, mut wh) = tokio::io::split(stream);
                check_conn(&mut rh, &mut wh, genesis_hash)
                    .await
                    .map_err(|e| TofuError::Other(e))
...
async fn connect_with_tofu(
    ...
    let tofu_verifier = Arc::new(TofuCertVerifier::new(
        base_verifier,
        Arc::new((**trusted_certificates).clone()),
    ));
    ...
    match connector.connect(dnsname.clone(), sock).await {
        Ok(stream) => {
            // Check if there's a certificate to pin
            if let Some(cert) = tofu_verifier.take_cert_to_pin(host) {
                tracing::info!("Auto-pinning PKI-validated certificate for {}", host);
                let mut db_guard = db.lock().unwrap();
                if let Err(e) = trusted_certificates.mutate2(&mut *db_guard, |trusted_certs, update| {
                    trusted_certs.add_certificate(cert.clone(), host.to_string(), update);
                    Ok(())
                }) {
                    tracing::error!("Failed to auto-pin PKI certificate: {:?}", e);
                }
            }
            Ok(stream)
        },
        Err(e) => {
            ...
            if let Some(tofu_error) = tofu_verifier.take_tofu_error(host) {
                tracing::info!("TLS connection rejected due to TOFU verification: {:?}", tofu_error);
                Err(tofu_error)
            } else {
                // No TOFU error stored, return the rustls error
                tracing::error!("TLS handshake failed for {}: {}", host, e);
                Err(TofuError::Other(anyhow!("TLS handshake failed: {}", e)))
            }
        }
    }
}
