...
///
/// Ownership is taken from the PSBT's own key origins on both sides, and every claimed path
/// is derived from `master_appkey` and matched against the actual script before it counts,
/// so a PSBT cannot make us attribute a script to a key that does not produce it.
///
    ...
    for (i, input) in psbt.inputs.iter().enumerate() {
        ...
        let bip32_path = match claimed_path(
            ...
            Err(NotOurs::Hardened) => {
                return Err(anyhow!("can't sign with hardened derivation").into())
            }
...
mod test {
    ...
    fn already_signed_input_is_foreign_and_tallied() {
        ...
        let err = psbt_to_tx_template(&psbt, key).unwrap_err();
