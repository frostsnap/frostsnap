...
impl CdcAcmSerial {
    ...
    ///
    /// Create a blocking CDC-ACM port from an already-open usbfs FD.
    pub fn new_auto(fd: OwnedFd, name: String, baud: u32) -> Result<Self> {
        let dev = Device::from_fd(fd).map_err(to_io)?;
        let cfg = dev.active_configuration().map_err(to_io)?;
        ...
        let if_comm = dev.detach_and_claim_interface(comm_if).map_err(to_io)?;
        event!(Level::DEBUG, if_num = data_if, "claiming data interface");
        let if_data = dev.detach_and_claim_interface(data_if).map_err(to_io)?;
...
fn send_cdc_setup(if_comm: &Interface, baud: u32) -> Result<()> {
    ...
    if_comm
        .control_out_blocking(ctl, &line_coding, Duration::from_millis(100))
        .map_err(to_io)?;
    ...
    if_comm
        .control_out_blocking(ctl, &[], Duration::from_millis(100))
        .map_err(to_io)?;
    ...
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        match block_on(select(rx_fut, delay_future)) {
            Either::Left((req, _unused_delay)) => {
                let data = req.into_result().map_err(to_io)?;
                rx_buf.extend(data);
                let n = rx_buf.len().min(buf.len());
                for i in 0..n {
                    buf[i] = rx_buf.pop_front().unwrap();
...
//--------------------------------------------------------------------------
// Helper: convert any nusb::Error or TransferError into std::io::Error
//--------------------------------------------------------------------------
