...
async fn test_electrum_frostsn_app_ssl_connection() {
    ...
    // Initialize TrustedCertificates - this should add the pre-trusted cert
    let trusted_certs = TrustedCertificates::initialize(&mut conn, ()).unwrap();
    
    // Verify that electrum.frostsn.app is pre-trusted
    assert!(
        trusted_certs.get_certificate_for_server("electrum.frostsn.app:50002").is_some(),
        "electrum.frostsn.app:50002 should be pre-trusted"
    );
}
...
async fn test_ssl_connection_with_letsencrypt_ca() {
    ...
    let _tls_stream = connector.connect(dnsname, stream).await.unwrap_or_else(|e| {
...
async fn test_emzy_electrum_server() {
    ...
    match connector.connect(dnsname, stream).await {
        Ok(_) => {
            println!("Successfully connected to {} (note: would need certificate in store)", addr);
        }
        Err(e) => {
            println!("Expected failure - Emzy's self-signed cert not in our store: {:?}", e);
        }
    }
}