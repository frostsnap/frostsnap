...
impl<I2C> CST816S<I2C, esp_hal::gpio::Input<'static>, esp_hal::gpio::Output<'static>> {
    ...
    pub fn new_esp32(
        ...
        use esp_hal::gpio::{Input, Output, Level, Pull, Event};
...
pub mod interrupt {
    ...
    pub fn register(
        ...
        unsafe {
            ...
            let (producer, consumer) = (*(&raw mut EVENT_QUEUE)).split();
            *(&raw mut PRODUCER) = Some(producer);
            
            // Store the instance (only accessed from interrupt)
            GLOBAL_INSTANCE = Some(instance);
    ...
    pub fn gpio_interrupt_handler() {
        unsafe {
            if let Some(cst) = (&raw mut GLOBAL_INSTANCE).as_mut().unwrap().as_mut() {
                // Clear the interrupt flag through the instance's pin
                cst.pin_int.clear_interrupt();
                
                // Read the touch event (this also clears the interrupt)
                if let Some(event) = cst.read_one_touch_event(false) {
                    // Try to enqueue the event (drops if queue is full)
                    if let Some(producer) = (&raw mut PRODUCER).as_mut().unwrap().as_mut() {
