...
impl<I2C> CST816S<I2C, esp_hal::gpio::Input<'static>, esp_hal::gpio::Output<'static>> {
    ...
    pub fn new_esp32(
        ...
        // Using LowLevel instead of FallingEdge to ensure we don't miss touch events
        // if multiple touches are buffered while INT stays LOW
    ...
    pub fn gpio_interrupt_handler() {
        unsafe {
            if let Some(cst) = (&raw mut GLOBAL_INSTANCE).as_mut().unwrap().as_mut() {
                ...
                if let Some(event) = cst.read_one_touch_event(true /* to avoid hangs */) {
