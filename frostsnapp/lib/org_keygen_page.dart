...
extension _ResolvedKeygenDeviceCount on ResolvedKeygen {
  int totalDeviceCount() =>
      participants.fold(0, (sum, p) => sum + p.devices.length);
}
...
class LobbyAndKeygenController extends ChangeNotifier {
  ...
  Future<void> startKeygenCeremony() async {
    ...
    try {
      ...
      _keygenStateSub = session.subState().watch().listen((state) {
        _keygenState = state;
        notifyListeners();
      });
      notifyListeners();
          ...
          children: [
            ...
            if (pending != null)
              _AcceptInfoRow(
                ...
                label: 'Threshold',
                value:
                    '${pending.threshold} of ${pending.totalDeviceCount()} required to spend',
              ),
            if (showNetwork)
              Padding(
          ...
          children: [
            if (pending != null)
              _ThresholdHero(
                ...
                total: pending.totalDeviceCount(),
              ),
            const SizedBox(height: 12),
            if (state != null && pending != null)
              ..._ackParticipantRows(
              ...
              Card.filled(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        '${pending.threshold}-of-${pending.totalDeviceCount()}',
                        style: theme.textTheme.labelLarge,
                      ),
                      Text(
