...
class WalletCreateController extends ChangeNotifier {
  ...
  TextSpan get subtitle => switch (_step) {
    ...
    WalletCreateStep.deviceNames => TextSpan(
      text: 'Names will be used to identify each device of ',
      children: [
        TextSpan(
          text: _form.name ?? '',
          style: TextStyle(fontStyle: FontStyle.italic),
...
class _WalletCreatePageState extends State<WalletCreatePage> {
  ...
  Widget buildNameDevicesBody(BuildContext context) {
    final theme = Theme.of(context);
    final selectedDevices = _controller.form.selectedDevices;
    return MultiSliver(
      children: [
        SliverDeviceList(
          deviceBuilder: (context, device) {
            final isPartOfWallet = selectedDevices.contains(device.id);
            final onPressed =
                isPartOfWallet
                    ? () => showRenameDeviceDialog(context, device)
                    : null;
            return buildDevice(
              context,
              device,
              rightPadding: isPartOfWallet ? 4 : null,
              onPressed: onPressed,
              enabled: isPartOfWallet,
              trailing:
                  isPartOfWallet
                      ? buildDeviceTrailingRename(
                        context,
                        device,
                        onPressed: onPressed,
                      )
