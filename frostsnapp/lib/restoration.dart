...
class WalletRecoveryPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final threshold = status.threshold;

    // When not recovered, we should count unique share indices regardless of compatibility
    // since compatibility isn't determined until we find a valid subset
    final uniqueIndices = status.shares.map((s) => s.index).toSet().length;

    final hasIncompatibleShares =
        !isRecovered && threshold != null && uniqueIndices >= threshold;
    final sharesNeeded = threshold != null && !isRecovered
        ? threshold - uniqueIndices
        : null;
    final isReady = isRecovered;

    final progressActionCard = MaterialDialogCard(
      iconData: isReady
          ? Icons.check_circle_outline_rounded
          : hasIncompatibleShares
          ? Icons.running_with_errors_rounded
          : Icons.info_rounded,
      title: isReady
          ? Text('Ready to restore')
          : hasIncompatibleShares
          ? Text('Some shares are invalid')
          : threshold == null
          ? Text('Gathering keys')
      ...
      content: isReady
          ...
          : threshold == null
          ? Text('Add more keys to restore the wallet.')
          : sharesNeeded != null
          ? Text(
              sharesNeeded == 1
          ...
          : Text('Add more keys to restore wallet.'),
    ...
    return Focus(
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            Text(
              "If you know the threshold from your backup, enter it here.\nOtherwise, we’ll determine it automatically from the keys you provide.",
              style: theme.textTheme.bodyMedium,
            ),
            const SizedBox(height: 24),
