...
class WalletModeShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final asRef = walletItem.frostKey
        .accessStructures()[0]
        .accessStructureRef();
            ...
            .map((unbroadcastedTx) {
              ...
              if (session != null) {
                ...
                return TxSentOrReceivedTile(
                  ...
                  txDetails: txDetails,
                  mode: TxTileSigning(signingState),
                );
              } else {
                final signedTx = unbroadcastedTx.signedTx();
                if (signedTx == null) {
                  // Finished session with no Bitcoin task — can't render
                  // as broadcastable, but it's also not a history row
                  // (it's a pending non-broadcast artifact like a message
                  // signature). Render as history; not tappable.
                  return TxSentOrReceivedTile(txDetails: txDetails);
                }
                return TxSentOrReceivedTile(
                  onTap: () => showBottomSheetOrDialog(
                    context,
                  ...
                  txDetails: txDetails,
                  mode: TxTileBroadcastable(signedTx),
                );
              }
            });
...
class WalletBottomBar extends StatelessWidget {
  ...
  Future<void> showTxDetailsDialog(
    ...
    if (session != null) {
      ...
      final signedTx = unbroadcastedTx.signedTx();
      if (signedTx == null) {
        // Finished session with no Bitcoin task — nothing to broadcast.
        return;
      }
      await showBottomSheetOrDialog(
        context,
            ...
            SliverSafeArea(
              sliver: SliverList.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  ...
                  final activeState = uncanonicalTx.activeSession?.state();
                  final signed = uncanonicalTx.signedTx();
                  final TxTileMode tileMode;
                  if (activeState != null) {
                    tileMode = TxTileSigning(activeState);
                  } else if (signed != null) {
                    tileMode = TxTileBroadcastable(signed);
                  } else {
                    tileMode = const TxTileHistory();
                  }
                  return TxSentOrReceivedTile(
                    txDetails: txDetails,
                    mode: tileMode,
