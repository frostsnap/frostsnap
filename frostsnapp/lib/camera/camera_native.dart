...
class _NativeCameraWidgetState<T> extends State<NativeCameraWidget<T>> {
  ...
  Future<void> _startCamera(camera.CameraDevice device) async {
    ...
    try {
      final eventStream = device.start();

      _cameraSubscription = eventStream.listen((event) {
        if (_finishedScanning) return;

        switch (event) {
          case camera.CameraEvent_Frame(:final field0):
            _handleFrame(field0);
            break;
          case camera.CameraEvent_QrDetected():
            // Not used
            break;
          case camera.CameraEvent_Error(:final field0):
            if (mounted && !_finishedScanning) {
              setState(() => _error = field0);
            }
            break;
        }
      });
  ...
  Future<void> _handleFrame(camera.CameraFrame rustFrame) async {
    if (_finishedScanning) return;

    setState(() {
      _latestFrame = Uint8List.fromList(rustFrame.data);
    });

    try {
      final frame = CameraFrame(
        data: rustFrame.data,
        width: rustFrame.width.toInt(),
        height: rustFrame.height.toInt(),
      );
