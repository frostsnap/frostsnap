...
pub use frostsnap_coordinator::bitcoin::chain_sync::{ChainStatus, ChainStatusState, ConnectionResult};
...
impl Settings {
    pub(crate) fn new(
        ...
        for network in SUPPORTED_NETWORKS {
            ...
            let trusted_certificates = {
                ...
                use frostsnap_coordinator::persist::Persisted;
                Persisted::<TrustedCertificates>::new(&mut *db_, ())?
            };
            
            let (chain_api, conn_handler) = ChainClient::new(genesis_hash, trusted_certificates, db.clone());
            let super_wallet =
    ...
    pub fn accept_certificate_and_retry(
        ...
        // Force reconnect to use the new certificate
        chain_api.reconnect();
