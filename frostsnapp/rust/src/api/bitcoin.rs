...
impl Transaction {
    ...
    pub(crate) fn fill_signatures(&mut self, signatures: &[EncodedSignature]) {
        assert_eq!(
            signatures.len(),
            self.owned_spends.len(),
            "one signature per owned input"
        );
        for (owned, signature) in self.owned_spends.iter().zip(signatures) {
            ...
            let witness = match &owned.script {
                ...
                // Script-path witness: [signature, leaf_script, control_block].
    ...
    /// An empty list means the witnesses were already filled in, as when restoring a finished
    /// signing session.
