...
#[cfg(any(target_os = "linux", target_os = "windows"))]
impl simplecam::CameraSink for CameraSinkAdapter {
    fn send_frame(&self, simplecam_frame: simplecam::Frame) -> simplecam::Result<()> {
        let frame = Frame {
            data: simplecam_frame.data,
            width: simplecam_frame.width,
            height: simplecam_frame.height,
        };

        self.sink.add(frame).map_err(|_| {
            info!("Camera '{}' stream closed, stopping capture", self.name);
            simplecam::CameraError::CaptureFailed("Stream closed".to_string())
        })
    }
}
