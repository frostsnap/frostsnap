use crate::api::port::{FfiSerial, PortEvent};
...
impl Serial for FfiSerial {
    ...
    fn open_device_port(&self, id: &str, baud_rate: u32) -> Result<SerialPort, PortOpenError> {
        ...
        crate::api::port::emit_event(dbg!(PortEvent::Open {
            request: crate::api::port::PortOpen {
                id: id.into(),
                baud_rate,
                ready: tx,
            },
...
impl io::Read for FfiSerialPort {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        loop {
            ...
            crate::api::port::emit_event(PortEvent::Read {
                request: crate::api::port::PortRead {
                    id: self.id.clone(),
                    len: buf.len() as u32,
                    ready: tx,
                },
...
impl std::io::Write for FfiSerialPort {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        crate::api::port::emit_event(PortEvent::Write {
            request: crate::api::port::PortWrite {
                id: self.id.clone(),
                bytes: buf.to_vec(),
                ready: tx,
            },
...
mod _impl {
    ...
    impl SerialPort for super::FfiSerialPort {
        ...
        fn bytes_to_read(&self) -> Result<u32> {
            ...
            crate::api::port::emit_event(PortEvent::BytesToRead {
                request: crate::api::port::PortBytesToRead {
                    id: self.id.clone(),
                    ready: tx,
                },
            })
