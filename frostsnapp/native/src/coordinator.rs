...
impl FfiCoordinator {
    ...
    pub fn active_signing_session(
        ...
        let coord = self.coordinator.lock().unwrap();
        SyncReturn(|| -> Option<api::DetailedSigningState> {
            let session = coord.active_signing_sessions().get(&session_id)?;
    ...
    pub fn active_signing_sessions(
        ...
        let sessions = coord
            .active_signing_sessions()
            .iter()
            .inspect(|&(&id, _)| event!(Level::DEBUG, "Found active signing session: {}", id))
            .filter(|(_, session)| session.key_id == key_id)
            .filter_map(|(id, session)| -> Option<api::DetailedSigningState> {
                Self::_active_signing_session_details(
                    coord
    ...
    pub fn cancel_sign_session(&self, ssid: SignSessionId) -> anyhow::Result<()> {
        ...
        if let Some(session) = coord.active_signing_sessions().get(&ssid) {
