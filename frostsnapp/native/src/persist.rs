...
use frostsnap_coordinator::{
    frostsnap_core::{
        self, message::CoordinatorToStorageMessage, CoordinatorFrostKey, DeviceId,
        FrostCoordinator, KeyId, SignTask,
    },
    persist::Persist,
    UiToStorageMessage,
};
...
impl Persist<rusqlite::Connection> for FrostCoordinator {
    type Update = CoordinatorToStorageMessage;
    type InitParams = ();
    ...
    {
        conn.execute(
            "CREATE TABLE IF NOT EXISTS fs_keys (
        ...
        conn.execute(
            "CREATE TABLE IF NOT EXISTS fs_nonces (
...
pub struct DeviceNames(BTreeMap<DeviceId, String>);
...
impl Persist<rusqlite::Connection> for DeviceNames {
    ...
    fn initialize(db: &mut rusqlite::Connection, params: Self::InitParams) -> anyhow::Result<Self>
    where
        Self: Sized,
    {
        todo!()
