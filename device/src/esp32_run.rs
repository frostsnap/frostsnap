    ...
    Rng: RngCore,
{
    pub fn run(self) -> ! {
        ...
        let mut upstream_read_magic_bytes = false;
        ...
        loop {
            if soft_reset {
                ...
                upstream_read_magic_bytes = false;
                outbox.clear();
            ...
            if !upstream_read_magic_bytes {
                upstream_read_magic_bytes = upstream_serial.find_and_remove_magic_bytes();
                if upstream_read_magic_bytes {
                    ui.set_upstream_connection_state(UpstreamConnectionState::Established {
                        is_device: !upstream_serial.is_jtag(),
                    })
                }
            }

            if upstream_read_magic_bytes && !upstream_sent_magic_bytes {
                upstream_serial
                    .write_magic_bytes()
