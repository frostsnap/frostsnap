...
pub fn run<'a>(resources: &'a mut Resources<'a>) -> ! {
    ...
    let mut next_write_magic_bytes_downstream = esp_hal::time::Instant::now();
    ...
    loop {
        if soft_reset {
            ...
            next_write_magic_bytes_downstream = esp_hal::time::Instant::now();
            ui.set_workflow(default_workflow!(name, signer));
            upgrade = None;
            ...
            (true, DownstreamConnectionState::Connected) => {
                ...
                if now > next_write_magic_bytes_downstream {
                    next_write_magic_bytes_downstream =
                        now + esp_hal::time::Duration::from_millis(MAGIC_BYTES_PERIOD as u64);
                    downstream_serial
                        .write_magic_bytes()
        ...
        for message_body in inbox.drain(..) {
            match &message_body {
                ...
                CoordinatorSendBody::Challenge(challenge) => {
                    ...
                    {
                        let signature = match hw_rsa.0.sign(
                            challenge.as_ref(),
                            sha256,
                            esp_hal::ds::KeySize::Rsa3072,
                            &hw_rsa.1,
                        ) {
                            Ok(esp_hal::ds::Signature::Rsa3072(signature)) => signature,
                            Ok(_) => panic!("unexpected DS signature size"),
                            Err(err) => panic!("DS signing failed: {err:?}"),
                        };
