...
impl<'a> EfuseController<'a> {
    ...
    pub fn discover_efuses(&self) -> DiscoveredEfuses {
        ...
        // Scan all key slots
        for key_id in [KeyId::Key0, KeyId::Key1, KeyId::Key2, KeyId::Key3, KeyId::Key4, KeyId::Key5] {
    ...
    unsafe fn set_efuse_key(&self, key_id: esp_hal::hmac::KeyId, value: [u8; 32]) -> Result<(), EfuseError> {
...
impl<'a> EfuseKeyWriter<'a> {
    ...
    pub fn add_key(mut self, key_id: esp_hal::hmac::KeyId, value: [u8; 32], purpose: KeyPurpose) -> Self {
    ...
    pub fn add_encryption_key(mut self, value: [u8; 32]) -> Self {
        self.keys.push((EfuseHmacKeys::ENCRYPTION_KEYID, value, KeyPurpose::HmacUpstream));
        self
    }

    /// Add the fixed entropy key
    pub fn add_entropy_key(mut self, value: [u8; 32]) -> Self {
        self.keys.push((EfuseHmacKeys::FIXED_ENTROPY_KEYID, value, KeyPurpose::HmacUpstream));
        self
    }

    /// Add the DS (Digital Signature) key for hardware attestation
    pub fn add_ds_key(mut self, value: [u8; 32]) -> Self {
        self.keys.push((EfuseHmacKeys::DS_KEYID, value, KeyPurpose::Ds));
        self
    }
    ...
    pub fn write_efuses(self) -> Result<(), EfuseError> {
        ...
        let configs: Vec<(u8, KeyPurpose, bool)> = self.keys
            .iter()
