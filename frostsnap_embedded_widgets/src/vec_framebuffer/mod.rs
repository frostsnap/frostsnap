...
use embedded_graphics::{
    ...
    pixelcolor::{
        raw::{RawData, RawU16, RawU8, RawU4, RawU2, RawU1},
        PixelColor, Rgb565, Gray8, Gray4, Gray2, BinaryColor,
    },
    ...
    C: PixelColor,
{
    ...
    pub fn buffer_size(width: usize, height: usize) -> usize {
        let bits_per_pixel = C::Raw::BITS_PER_PIXEL as usize;
    ...
    pub fn get_pixel(&self, point: Point) -> Option<Rgb565> {
        if let (Ok(x), Ok(y)) = (usize::try_from(point.x), usize::try_from(point.y)) {
            if x < self.width && y < self.height {
                ...
                // Little endian: low byte first
                let value = self.data[byte_index] as u16 
                          | ((self.data[byte_index + 1] as u16) << 8);
