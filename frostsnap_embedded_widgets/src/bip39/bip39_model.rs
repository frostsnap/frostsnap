...
use alloc::vec::Vec;
use frostsnap_backup::bip39_words::{self, ValidLetters, FROSTSNAP_BACKUP_WORDS};

#[derive(Debug)]
pub struct Bip39Model {
    share_index: String,
    share_index_confirmed: bool,
    words: [Cow<'static, str>; FROSTSNAP_BACKUP_WORDS],
}
...
impl Bip39Model {
    pub fn new() -> Self {
        Self {
            ...
            words: [const { Cow::Borrowed("") }; FROSTSNAP_BACKUP_WORDS],
        }
    }
    ...
    pub fn backspace(&mut self) -> Vec<FramebufferMutation> {
        ...
        if current.is_empty() {
            ...
            if row == 0 {
                ...
                if prev_word_idx < FROSTSNAP_BACKUP_WORDS {
    ...
    fn get_words_as_static(&self) -> Option<Box<[&'static str; FROSTSNAP_BACKUP_WORDS]>> {
        // Only return if all words are valid BIP39 words (Borrowed variants)
        if !self.is_complete() {
            return None;
        }

        let mut static_words = [""; FROSTSNAP_BACKUP_WORDS];
