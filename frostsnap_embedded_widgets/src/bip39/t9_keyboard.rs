...
impl T9Keyboard {
    ...
    pub fn get_pending_char(&self) -> Option<char> {
        if let Some(ref state) = self.multi_tap_state {
            ...
            if valid_letters.is_empty() {
                ...
                valid_letters.chars().nth(state.tap_count % valid_letters.len())
            }
        } else {
            None
        }
        ...
        for (row_idx, row) in self.key_rects.iter().enumerate() {
            for (col_idx, rect) in row.iter().enumerate() {
                if rect.contains(point) {
                    return Some(T9KeyPosition { row: row_idx, col: col_idx });
                }
            }
        }
        None
    ...
    pub fn handle_key_press(&mut self, point: Point, current_time: crate::Instant) -> Option<(Key, Rectangle)> {
    ...
    fn draw<D: DrawTarget<Color = Rgb565>>(
        ...
        for (row_idx, row) in self.key_rects.iter().enumerate() {
            for (col_idx, rect) in row.iter().enumerate() {
                ...
                let is_active = self.multi_tap_state.as_ref()
