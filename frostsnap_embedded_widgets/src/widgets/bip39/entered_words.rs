...
use crate::widgets::{Key, KeyTouch};
use alloc::rc::Rc;
...
use super::{
    ...
    scroll_bar::{ScrollBar, SCROLLBAR_WIDTH},
...
const TOTAL_CONTENT_HEIGHT: u32 = TOP_PADDING + (TOTAL_WORDS as u32 * (FONT_SIZE.height + VERTICAL_PAD));
...
impl EnteredWords {
    ...
    pub fn draw<D: DrawTarget<Color = Rgb565>>(&mut self, target: &mut D) {
        ...
        if self.scroll_position < TOTAL_CONTENT_HEIGHT as i32 {
            ...
            let words_visible_height =
                (TOTAL_CONTENT_HEIGHT as i32 - self.scroll_position).min(scrollable_height) as usize;
    ...
    pub fn handle_touch(&self, point: Point) -> Option<KeyTouch> {
        ...
        if point.x >= WORD_LIST_LEFT_PAD && point.x < WORD_LIST_LEFT_PAD + FB_WIDTH as i32 {
            ...
            let content_point = Point::new(point.x - WORD_LIST_LEFT_PAD, point.y + self.scroll_position);
            ...
            if word_index < TOTAL_WORDS {
                ...
                let y =
                    TOP_PADDING as i32 + (word_index as i32 * row_height) - self.scroll_position;
                let button_y = self.visible_size.height as i32 - SUBMIT_BUTTON_HEIGHT as i32;
