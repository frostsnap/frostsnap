...
use super::slide_iterator::{SlideDirection, SlideIterator};
...
impl PageTransitionHandler {
    ...
    pub fn draw<D: DrawTarget<Color = Rgb565>>(
        ...
        let (progress_millis, direction) = match &self.animation {
            Some(anim) => {
                let elapsed = anim
                    .start_time
                    .and_then(|start| current_time.duration_since(start))
                    .unwrap_or(0);
                // Scale to 0-1000 range, clamped to animation duration
                let progress = if elapsed >= ANIMATION_DURATION_MS {
                    1000
                } else {
                    ((elapsed * 1000) / ANIMATION_DURATION_MS) as usize
                };
                (progress, anim.direction)
            }
            None => (0, SlideDirection::Left), // Default for init_draw
        };

        // Calculate how many pixels to take from next framebuffer
        // next_pixels = width * progress_millis / 1000
        let next_pixels = (self.width * progress_millis) / 1000;
