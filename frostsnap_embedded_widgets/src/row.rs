use crate::{Widget, Instant};
use embedded_graphics::{
    draw_target::DrawTarget,
    geometry::{Point, Size},
    pixelcolor::{PixelColor, Rgb565},
    prelude::*,
    primitives::Rectangle,
};

/// A row widget that arranges its children horizontally
pub struct Row<T, C = Rgb565> {
    children: T,
    _phantom: core::marker::PhantomData<C>,
}

impl<T, C> Row<T, C> {
    pub fn new(children: T) -> Self {
        Self { 
            children,
            _phantom: core::marker::PhantomData,
        }
    }
}

// Macro to implement Widget for Row with tuples of different sizes
macro_rules! impl_row_for_tuple {
    // Base case for a single element
    ($t1:ident) => {
        impl<$t1: Widget<Color = C>, C: PixelColor> Widget for Row<($t1,), C> {
            type Color = C;
            
            fn draw<D: DrawTarget<Color = C>>(
                &mut self,
                target: &mut D,
                current_time: Instant,
            ) -> Result<(), D::Error> {
                let x_offset = 0;
                
                let size = self.children.0.size_hint()
                    .expect("Row requires all children to have size_hint");
                let area = Rectangle::new(Point::new(x_offset, 0), size);
                let mut cropped = target.cropped(&area);
                self.children.0.draw(&mut cropped, current_time)?;
                
                Ok(())
            }
            
            fn handle_touch(
                &mut self,
                point: Point,
                current_time: Instant,
                is_release: bool,
            ) -> Option<crate::KeyTouch> {
                let x_offset = 0;
                
                let size = self.children.0.size_hint()
                    .expect("Row requires all children to have size_hint");
                let area = Rectangle::new(Point::new(x_offset, 0), size);
                
                if area.contains(point) {
                    let relative_point = Point::new(point.x - x_offset, point.y);
                    return self.children.0.handle_touch(relative_point, current_time, is_release);
                }
                
                None
            }
            
            fn handle_vertical_drag(&mut self, start_y: Option<u32>, current_y: u32) {
                // For now, pass drag to all children - could be improved
                self.children.0.handle_vertical_drag(start_y, current_y);
            }
            
            fn size_hint(&self) -> Option<Size> {
                let size = self.children.0.size_hint()?;
                Some(Size::new(size.width, size.height))
            }
        }
    };
    
    // Recursive case for multiple elements
    ($t1:ident, $($tn:ident),+) => {
        impl<$t1: Widget<Color = C>, $($tn: Widget<Color = C>),+, C: PixelColor> Widget for Row<($t1, $($tn,)+), C> {
            type Color = C;
            
            #[allow(unused_assignments)]
            fn draw<D: DrawTarget<Color = C>>(
                &mut self,
                target: &mut D,
                current_time: Instant,
            ) -> Result<(), D::Error> {
                let mut x_offset = 0;
                
                // Destructure the tuple
                #[allow(non_snake_case)]
                let (ref mut $t1, $(ref mut $tn,)+) = self.children;
                
                // Draw first child
                {
                    let size = $t1.size_hint()
                        .expect("Row requires all children to have size_hint");
                    let area = Rectangle::new(Point::new(x_offset, 0), size);
                    let mut cropped = target.cropped(&area);
                    $t1.draw(&mut cropped, current_time)?;
                    x_offset += size.width as i32;
                }
                
                // Draw remaining children
                $(
                    {
                        let size = $tn.size_hint()
                            .expect("Row requires all children to have size_hint");
                        let area = Rectangle::new(Point::new(x_offset, 0), size);
                        let mut cropped = target.cropped(&area);
                        $tn.draw(&mut cropped, current_time)?;
                        x_offset += size.width as i32;
                    }
                )+
                
                Ok(())
            }
            
            #[allow(unused_assignments)]
            fn handle_touch(
                &mut self,
                point: Point,
                current_time: Instant,
                is_release: bool,
            ) -> Option<crate::KeyTouch> {
                let mut x_offset = 0;
                
                // Destructure the tuple
                #[allow(non_snake_case)]
                let (ref mut $t1, $(ref mut $tn,)+) = self.children;
                
                // Check first child
                {
                    let size = $t1.size_hint()
                        .expect("Row requires all children to have size_hint");
                    let area = Rectangle::new(Point::new(x_offset, 0), size);
                    
                    if area.contains(point) {
                        let relative_point = Point::new(point.x - x_offset, point.y);
                        return $t1.handle_touch(relative_point, current_time, is_release);
                    }
                    x_offset += size.width as i32;
                }
                
                // Check remaining children
                $(
                    {
                        let size = $tn.size_hint()
                            .expect("Row requires all children to have size_hint");
                        let area = Rectangle::new(Point::new(x_offset, 0), size);
                        
                        if area.contains(point) {
                            let relative_point = Point::new(point.x - x_offset, point.y);
                            return $tn.handle_touch(relative_point, current_time, is_release);
                        }
                        x_offset += size.width as i32;
                    }
                )+
                
                None
            }
            
            fn handle_vertical_drag(&mut self, start_y: Option<u32>, current_y: u32) {
                // For now, pass drag to all children - could be improved to only send to relevant child
                #[allow(non_snake_case)]
                let (ref mut $t1, $(ref mut $tn,)+) = self.children;
                
                $t1.handle_vertical_drag(start_y, current_y);
                $(
                    $tn.handle_vertical_drag(start_y, current_y);
                )+
            }
            
            fn size_hint(&self) -> Option<Size> {
                #[allow(non_snake_case)]
                let (ref $t1, $(ref $tn,)+) = self.children;
                
                // Calculate total width and maximum height
                let mut total_width = 0;
                let mut max_height = 0;
                
                // First child
                let size = $t1.size_hint()?;
                total_width += size.width;
                max_height = max_height.max(size.height);
                
                // Remaining children
                $(
                    let size = $tn.size_hint()?;
                    total_width += size.width;
                    max_height = max_height.max(size.height);
                )+
                
                Some(Size::new(total_width, max_height))
            }
        }
    };
}

// Generate implementations for tuples up to 9 elements
impl_row_for_tuple!(T1);
impl_row_for_tuple!(T1, T2);
impl_row_for_tuple!(T1, T2, T3);
impl_row_for_tuple!(T1, T2, T3, T4);
impl_row_for_tuple!(T1, T2, T3, T4, T5);
impl_row_for_tuple!(T1, T2, T3, T4, T5, T6);
impl_row_for_tuple!(T1, T2, T3, T4, T5, T6, T7);
impl_row_for_tuple!(T1, T2, T3, T4, T5, T6, T7, T8);
impl_row_for_tuple!(T1, T2, T3, T4, T5, T6, T7, T8, T9);