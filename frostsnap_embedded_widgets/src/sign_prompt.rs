use crate::{
    any_of::AnyOf, bitcoin_amount_display::BitcoinAmountDisplay, center::Center, color_map::ColorMap, icons::IconWidget, Column, CrossAxisAlignment, MainAxisAlignment, page_slider::PageSlider, widget_list::WidgetList, sized_box::SizedBox, text::Text, DynWidget, Instant, Padding, Widget, HoldToConfirm, palette::PALETTE
};
...
type AddressDisplayWidget = AnyOf<(ColorMap<crate::p2tr_address_display::P2trAddressDisplay, Rgb565>, Text<U8g2TextStyle<Rgb565>>)>;
    ...
    fn new(index: usize, address: &bitcoin::Address) -> Self {
        ...
        let address_display = match address.address_type() {
            Some(bitcoin::AddressType::P2tr) => {
                // P2TR address (Taproot) - needs color mapping from Gray4 to Rgb565
                let p2tr_display = crate::p2tr_address_display::P2trAddressDisplay::new(&address.to_string());
                let color_mapped = p2tr_display.color_map(|c| {
                    use embedded_graphics::prelude::GrayColor;
                    match c.luma() {
                        0 => PALETTE.background,
                        1..=3 => PALETTE.surface,
                        4..=5 => PALETTE.surface_variant,
                        6 => PALETTE.text_disabled,
                        7..=9 => PALETTE.text_secondary,
                        10..=12 => PALETTE.primary,
                        13..=15 => PALETTE.on_surface,
                        _ => PALETTE.background,
                    }
                });
                AddressDisplayWidget::new(color_mapped)
            }
            _ => {
...
pub struct WarningPage {
    #[widget_delegate]
    center: Center<Column<(
        IconWidget<embedded_iconoir::Icon<Rgb565, embedded_iconoir::icons::size48px::actions::WarningTriangle>>,
        Text<U8g2TextStyle<Rgb565>>,
        Text<U8g2TextStyle<Rgb565>>,
    )>>,
}
...
type SignPromptPage = AnyOf<(AmountPage, AddressPage, FeePage, WarningPage, ConfirmationPage)>;
...
impl WidgetList<SignPromptPage> for SignPromptPageList {
    ...
    fn get(&self, index: usize) -> Option<SignPromptPage> {
        ...
        let page = if index < recipient_pages {
            ...
            let total_sent: u64 = self.prompt.foreign_recipients
                .iter()
                .map(|(_, amount)| amount.to_sat())
                .sum();
