...
pub fn derive_widget(input: TokenStream) -> TokenStream {
    ...
    match &input.data {
        Data::Enum(data_enum) => {
            derive_widget_for_enum(name, &impl_generics, &ty_generics, &where_clause, data_enum, &crate_path)
        }
        Data::Struct(data_struct) => {
            derive_widget_for_struct(name, &impl_generics, &ty_generics, &where_clause, data_struct, &input.attrs, &crate_path)
        }
        _ => {
            syn::Error::new_spanned(name, "Widget can only be derived for enums and structs")
                .to_compile_error()
                .into()
        }
    }
...
fn get_crate_path(attrs: &[syn::Attribute]) -> proc_macro2::TokenStream {
    ...
    // Default: try to detect if we're in frostsnap_embedded_widgets itself
    // by using crate:: which will work within the crate, and users outside
    // can specify #[widget_crate(frostsnap_embedded_widgets)]
    quote!(crate)
}
...
fn derive_widget_for_struct(
    ...
    let (delegate_field, field_type) = match find_delegate_field_with_type(&data_struct.fields, attrs) {
        Some(result) => result,
        None => {
...
fn find_delegate_field_with_type(fields: &syn::Fields, struct_attrs: &[syn::Attribute]) -> Option<(proc_macro2::TokenStream, syn::Type)> {
