...
impl ChannelSecret {
    ...
    pub fn from_access_structure_id(id: &AccessStructureId) -> Self {
        ...
        let hash = prefix_hash("NOSTR_CHANNEL_SECRET/5", &id.0);
        let mut secret = [0u8; 16];
    ...
    /// Invite link for a *keygen lobby* — a separate scheme
    /// (`frostsnap://keygen/<hex>`) from the post-keygen wallet channel
    /// link so the receiver UI can route to the right flow.
...
/// subscriptions. Distinct from `frostsnap_core::ChannelId`, which identifies
/// a keygen/signing session *within* such a channel.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct NostrChannelId(pub [u8; 32]);
...
impl ChannelKeys {
    pub fn from_channel_secret(secret: &ChannelSecret) -> Self {
        let channel_id = NostrChannelId(prefix_hash("NOSTR_CHANNEL_ID", &secret.0));
...
impl ChannelInitData {
    ...
    /// Build the inner channel creation event for this init payload.
    /// This uses an ephemeral key because channel init doesn't need author
    /// attribution; the channel runner wraps and encrypts it.
    pub fn to_channel_creation_event(&self) -> Result<Event> {
        let encoded = bincode::encode_to_vec(self, crate::channel_runner::BINCODE_CONFIG)?;
...
/// Used for post-keygen wallet channel invites.
pub fn parse_frostsnap_link(url: &str) -> Result<ChannelSecret, String> {
    parse_with_prefix(url, "frostsnap://channel/")
}

/// Parse a `frostsnap://keygen/<channel_secret_hex>` link into a ChannelSecret.
...
fn parse_with_prefix(url: &str, prefix: &str) -> Result<ChannelSecret, String> {
    let hex_str = url
        .strip_prefix(prefix)
        .ok_or_else(|| format!("invalid frostsnap link: expected {prefix}<secret_hex>"))?;
...
mod tests {
    ...
    fn channel_id_and_shared_secret_are_different() {
        ...
        assert_ne!(keys.channel_id.0, keys.shared_secret);
    }
