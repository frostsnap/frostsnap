...
impl TransactionTemplate {
    ...
    /// The sighash for each input. Owned inputs use the sighash for how we spend them; foreign
    /// inputs get the key-path sighash but are never signed.
    pub fn iter_sighash(&self) -> Vec<TapSighash> {
        ...
        self.inputs
            ...
            .enumerate()
            .map(
                |(i, input)| match input.owner.local_owner().and_then(LocalSpk::leaf_hash) {
    ...
    pub fn user_prompt(&self, network: bitcoin::Network) -> PromptSignBitcoinTx {
        ...
        let spends_scripted_output = self
...
pub struct PromptSignBitcoinTx {
    ...
    pub spends_scripted_output: bool,
}
...
impl LocalSpk {
    ...
    /// True if this is anything other than a plain BIP86 key-spend.
...
mod test {
    ...
    /// key we sign with must be the raw (untweaked) key that actually appears in the leaf.
    #[test]
