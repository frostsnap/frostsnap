...
pub(super) enum RemoteKeygenPhase {
    WaitingForInputs {
        ...
        contributer: Box<certpedpop::Contributor<certpedpop::AuxContributor>>,
        my_keypair: KeyPair,
    },
...
fn resolve_party(
    ...
    devices_in_order: &[DeviceId],
    from: DeviceId,
) -> Result<(certpedpop::Party, bool), Error> {
    if let Some(coord_index) = coordinator_ids.iter().position(|id| *id == from) {
        return Ok((certpedpop::Party::AuxContributor(coord_index as u32), false));
    }
    if let Some(receiver_index) = devices_in_order.iter().position(|d| *d == from) {
        return Ok((certpedpop::Party::Receiver(receiver_index as u32), true));
    }
    ...
    match payload {
        RemoteKeygenPayload::Input(input) => {
            ...
            match input_aggregator.add_input(
                ...
                Ok(()) => {}
                Err(encpedpop::AddInputError::Inner(
                    simplepedpop::AddInputError::DuplicateInput { .. },
                )) => {
                    return Ok(vec![]);
                }
            ...
            if input_aggregator.is_finished() {
                ...
                let agg_input = input_aggregator
                    .finish_with_fingerprint::<Sha256>(keygen_fingerprint)
                ...
                let (verified, sig) = contributer
                    .verify_agg_input(&schnorr, &cert_scheme, agg_input.clone(), &my_keypair)
                ...
                state.phase = RemoteKeygenPhase::WaitingForCertificates { certifier };
            }
        ...
        RemoteKeygenPayload::Ack(session_hash) => {
            ...
            let all_acks_received = acks.len() == state.devices_in_order.len();
...
impl super::FrostCoordinator {
    /// Insert a key + master access structure obtained from outside the
    /// local keygen flow (e.g. handed off after a remote-coordinated
    /// keygen completes elsewhere). Wraps `mutate_new_key` with an empty
    /// device→share-index map because we are not the share holder.
    pub fn add_key_and_access_structure(
        &mut self,
        key_name: alloc::string::String,
        root_shared_key: SharedKey,
        purpose: super::KeyPurpose,
        encryption_key: crate::SymmetricKey,
        rng: &mut impl rand_core::RngCore,
    ) -> crate::AccessStructureRef {
        self.mutate_new_key(
            key_name,
            root_shared_key,
            BTreeMap::new(),
    ...
    pub fn begin_remote_keygen(
        ...
        input_aggregator
            .add_input(
                &schnorr,
                certpedpop::Party::AuxContributor(my_coordinator_index),
                input.clone(),
            )
