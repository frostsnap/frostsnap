...
impl State {
    ...
    /// session we also consume the local_nonces (binonces were exposed to the
...
pub enum RemoteSigningMutation {
    ...
    /// `Completed { sign_session_id, shares }`, and the reserved local_nonces are
    ...
    /// was `Used`, also consumes its local_nonces. `Open` sessions can be freely
    /// dropped (local_nonces never exposed); `Completed` sessions already had
    /// local_nonces consumed on completion.
    CancelRemoteSignSession { id: RemoteSignSessionId },
}

impl RemoteSigningMutation {
    pub fn tied_to_key(&self, coord: &super::FrostCoordinator) -> Option<crate::KeyId> {
        match self {
            RemoteSigningMutation::NewRemoteSignSession { session, .. } => {
                Some(session.access_structure_ref.key_id)
            }
            RemoteSigningMutation::UseRemoteSignSession { active, .. } => Some(active.key_id),
            RemoteSigningMutation::CompleteSigning { id, device_id, .. } => coord
                .remote_signing
                .remote_sign_sessions
                .get(&(*id, *device_id))
                .map(|s| s.access_structure_ref.key_id),
            RemoteSigningMutation::CancelRemoteSignSession { id } => coord
                .remote_signing
                .remote_sign_sessions
                .iter()
                .find(|((sid, _), _)| sid == id)
                .map(|(_, s)| s.access_structure_ref.key_id),
        }
    }
}
...
pub enum RemoteSignStatus {
    ...
    /// Signature share received and cached; local_nonces have been consumed.
    Completed {
        sign_session_id: SignSessionId,
        shares: ParticipantSignatureShares,
    },
...
impl FrostCoordinator {
    ...
    pub fn sign_with_nonce_reservation(
        ...
        let local_nonces_for_device = match &remote_session.status {
        ...
        Ok(RequestDeviceSign {
            ...
            request_sign: RequestSign {
                ...
                device_sign_req: DeviceSignReq {
                    nonces: local_nonces_for_device,
                    rootkey,
    ...
    /// Dispatch a `DeviceSigning::SignatureShare` whose `SignSessionId` was
    /// not found in `active_signing_sessions`. Handles both the share
    /// processing and the optional nonce-replenishment piggybacked in the
    /// same message, so the coordinator-side switch arm only has to detect
    /// the remote case and hand off.
    pub(super) fn receive_device_signature_share(
        &mut self,
        from: DeviceId,
        session_id: SignSessionId,
        signature_shares: &[SignatureShare],
        replenish_nonces: Option<&crate::nonce_stream::NonceStreamSegment>,
        message_kind: &'static str,
    ) -> MessageResult<Vec<CoordinatorSend>> {
        let outgoing =
            self.recv_remote_signature_share(from, session_id, signature_shares, message_kind)?;
        if let Some(replenish_nonces) = replenish_nonces {
            self.mutate(super::Mutation::Signing(
                super::SigningMutation::NewNonces {
                    device_id: from,
    ...
    /// `receive_device_signature_share` once we've decided this is a remote
    /// session. Finds the matching `Used` session by `SignSessionId`,
    /// verifies the share against its stored FROST context, emits a
    /// `CompleteSigning` mutation and a `GotShare` to-user message.
    pub fn recv_remote_signature_share(
        ...
        // local_nonces under this session. A share from any other device is bogus.
...
fn build_sign_sessions(
    ...
    Ok(sign_items
        ...
        .enumerate()
        .map(|(i, sign_item)| {
            let indexed_local_nonces = all_binonces
                .iter()
