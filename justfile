...
lint-device +ARGS="":
    ...
    cargo clippy --target riscv32imc-unknown-none-elf {{device_crates}} {{ARGS}} --all-features -- -Dwarnings
...
fix-rust:
    ...
    cargo clippy --target riscv32imc-unknown-none-elf --fix --allow-dirty --allow-staged {{device_crates}} --all-features
    cargo fmt --all
